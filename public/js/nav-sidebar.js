...
(function() {
  ...
  const navGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: 'timesheets', label: 'Timesheets', href: 'timesheets.html', section: 'Team' }
      ]
    }
  ];
    ...
    const navHTML = navGroups.map(group => {
      ...
      if (group.href) {
        ...
        Object.entries(sections).forEach(([sectionName, items]) => {
          dropdownItems += `<div class="dropdown-section-label">${sectionName}</div>`;
          items.forEach(item => {
            const itemActive = item.id === current.itemId;
            dropdownItems += `<a href="${item.href}" class="dropdown-menu-item ${itemActive ? 'active' : ''}">${item.label}</a>`;
          });
        });

        return `
          <div class="nav-dropdown nav-dropdown-business ${isActive ? 'active' : ''}">
            ...
            <div class="dropdown-menu dropdown-menu-sectioned">
              ${dropdownItems}
            </div>
          </div>
        `;
      } else {
        // Regular group with dropdown
  ...
  function init() {
    ...
    // Hide the sub-nav (we're using single-row dropdowns)
