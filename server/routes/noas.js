...
router.get('/', asyncHandler(async (req, res) => {
  ...
  let query = supabase.from('v2_noas').select('*, job:v2_jobs(id, name)').eq('builder_id', builderId).is('deleted_at', null).order('created_at', { ascending: false });
...
router.get('/stats', asyncHandler(async (req, res) => {
  ...
  let query = supabase.from('v2_noas').select('status, expiration_date, product_type').eq('builder_id', builderId).is('deleted_at', null);
  if (job_id) query = query.eq('job_id', job_id);
  const { data, error } = await query;
  if (error) throw error;
...
router.get('/search', asyncHandler(async (req, res) => {
  ...
  const { data, error } = await supabase.from('v2_noas').select('*, job:v2_jobs(id, name)').eq('builder_id', builderId).is('deleted_at', null).or('fl_approval_number.ilike.%' + searchTerm + '%,product_name.ilike.%' + searchTerm + '%,manufacturer.ilike.%' + searchTerm + '%').limit(50);
  if (error) throw error;
  res.json(data);
}));
...
router.get('/:id', asyncHandler(async (req, res) => {
  const builderId = getBuilderId(req);
  const { id } = req.params;
  const { data: noa, error: noaError } = await supabase.from('v2_noas').select('*, job:v2_jobs(id, name)').eq('id', id).eq('builder_id', builderId).is('deleted_at', null).single();
  if (noaError) throw noaError;
  if (!noa) return res.status(404).json({ error: 'NOA not found' });
  const { data: usages } = await supabase.from('v2_noa_usages').select('*, job:v2_jobs(id, name)').eq('builder_id', builderId).eq('noa_id', id).order('created_at', { ascending: false });
...
router.patch('/:id', asyncHandler(async (req, res) => {
  ...
  const { data, error } = await supabase.from('v2_noas').update(updates).eq('id', id).eq('builder_id', builderId).is('deleted_at', null).select('*, job:v2_jobs(id, name)').single();
  if (error) throw error;
...
router.delete('/:id', asyncHandler(async (req, res) => {
  const builderId = getBuilderId(req);
  const { id } = req.params;
  const { data, error } = await supabase.from('v2_noas').update({ deleted_at: new Date().toISOString() }).eq('id', id).eq('builder_id', builderId).is('deleted_at', null).select().single();
  if (error) throw error;
  if (!data) return res.status(404).json({ error: 'NOA not found' });
...
router.post('/:id/usages', asyncHandler(async (req, res) => {
  ...
  const { data, error } = await supabase.from('v2_noa_usages').insert({ builder_id: builderId, noa_id: id, job_id, location_description, quantity, notes }).select('*, job:v2_jobs(id, name)').single();
  if (error) throw error;
  res.status(201).json(data);
}));
...
router.patch('/:noaId/usages/:usageId', asyncHandler(async (req, res) => {
  ...
  const { data, error } = await supabase.from('v2_noa_usages').update(updates).eq('id', usageId).eq('builder_id', builderId).select('*, job:v2_jobs(id, name)').single();
  if (error) throw error;
...
router.post('/:noaId/usages/:usageId/verify', asyncHandler(async (req, res) => {
  ...
  const { data, error } = await supabase.from('v2_noa_usages').update({ verified_at: new Date().toISOString(), verified_by: getUserName(req) }).eq('id', usageId).eq('builder_id', builderId).select('*, job:v2_jobs(id, name)').single();
  if (error) throw error;
  if (!data) return res.status(404).json({ error: 'NOA usage not found' });
  res.json(data);
}));
...
router.delete('/:noaId/usages/:usageId', asyncHandler(async (req, res) => {
  const builderId = getBuilderId(req);
  const { usageId } = req.params;
  const { error } = await supabase.from('v2_noa_usages').delete().eq('id', usageId).eq('builder_id', builderId);
  if (error) throw error;
