...
must_haves:
  truths:
    ...
    - "v2_estimate_versions table exists for tracking estimate version history"
  artifacts:
    ...
    - path: "database/migration-117-estimating-data-model.sql"
      provides: "Version tracking table"
      contains: "CREATE TABLE.*v2_estimate_versions"
  key_links:
    ...
    - from: "v2_estimate_versions.estimate_id"
      to: "v2_estimates.id"
      via: "foreign key"
...
<objective>
Create database schema for hierarchical estimates with sections, assemblies, allowances, separate markup tracking, and version history.

Purpose: Establish the data foundation for the v4.0 estimating system rebuild, enabling proper organizational hierarchy (estimate -> sections -> items), reusable assembly templates, and version tracking for estimate history.
...
<task type="auto">
  <name>Task 3: Add separate markup columns, status constraint, and version tracking table</name>
  <files>database/migration-117-estimating-data-model.sql</files>
  <action>
Append to migration file:
...
2. Update status constraint to include 'sent' (EST-06 workflow):
```sql
-- ============================================================
...
3. Add version tracking table for estimate history (EST-06 versioning):
```sql
-- ============================================================
...
This covers EST-05 and EST-06 (both status workflow AND version tracking).
  </action>
  <verify>Run `npm run migrate` - migration should complete without errors. Verify with psql or Supabase dashboard that columns and v2_estimate_versions table exist.</verify>
  <done>Migration file complete with all schema changes including version tracking. Run migrate to apply.</done>
</task>
...
2. Verify tables exist:
   ...
   - Should show: v2_estimate_sections, v2_estimate_versions, v2_assembly_templates, v2_assembly_template_items (plus existing tables)
...
4. Verify new columns on v2_estimates:
   - Query: `SELECT column_name FROM information_schema.columns WHERE table_name = 'v2_estimates' AND column_name IN ('overhead_percent', 'overhead_amount', 'profit_percent', 'profit_amount', 'version');`
...
6. Verify v2_estimate_versions table exists:
   - Query: `SELECT column_name FROM information_schema.columns WHERE table_name = 'v2_estimate_versions';`
   - Should return: id, estimate_id, version_number, snapshot_data, created_at, created_by, change_summary, subtotal, total_amount, line_count
</verification>
...
7. v2_estimates has new columns: overhead_percent, overhead_amount, profit_percent, profit_amount, version
8. v2_estimates.status accepts 'sent' value
9. v2_estimate_versions table exists with estimate_id, version_number, snapshot_data, change_summary columns
</success_criteria>
