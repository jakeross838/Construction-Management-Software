...
export function generateCOPdf(co: COPdfData['co']): void {
  const doc = new jsPDF();
...
export function generateCOPdfBase64(co: COPdfData['co']): string {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  
  // Same content as generateCOPdf but returns base64
  doc.setFontSize(20);
  doc.setFont('helvetica', 'bold');
  doc.text('ROSS BUILT', 14, 20);
